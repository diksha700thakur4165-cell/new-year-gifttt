...
# --- BGM aur Styles (Everything is fixed here) ---
st.markdown("""
    <style>
    /* Sabhi text ko black aur bold karne ke liye */
    html, body, [data-testid="stWidgetLabel"], .stMarkdown, p, h1, h2, h3, h4, h5, h6, span, div {
    ...
    /* Buttons styling - Black text and Pink background */
    .stButton>button {
...
# --- Music Logic (Plays after first click to avoid browser blocks) ---
...
if st.session_state.page == 'envelope_back':
    ...
    st.markdown("<h2 style='text-align: center;'>Pls accept the gift Ishri Pishri ❤️</h2>", unsafe_allow_html=True)
...
# --- Box 1: New Year Wish ---
    ...
    for i in range(3, 0, -1):
        placeholder.markdown(f"<h1 style='text-align: center;'>Opening in... {i}</h1>", unsafe_allow_html=True)
        time.sleep(1)
    placeholder.empty()
    st.balloons()
    st.header("HAPPY NEW YEAR MY LOVE! 🎊")
    ...
    st.write("Har saal bas ek hi dua hai — tum mere saath raho. Tumhari hasi meri taqat hai, aur tumhara pyaar meri sabse badi jeet. Let's make this year amazing! ❤️")
    if st.button("Back 🔙"):
        st.session_state.page = 'gift_selection'
        st.rerun()
...
# --- Box 2: Certificate ---
    ...
    st.markdown(f"""
        <div style="border: 10px solid #FFD700; padding: 30px; text-align: center; background-color: white; border-radius: 15px; font-family: 'Georgia';">
            <h1 style="color: black !important; margin-bottom: 10px;">🏆 OFFICIAL CERTIFICATE 🏆</h1>
            <hr style="border: 1px solid #eee;">
            <p style="font-size: 20px;">This is proudly presented to:</p>
            <h1 style="text-decoration: underline;">Ishta Pathak</h1>
            <p style="font-size: 22px; font-style: italic;">For being the</p>
            <h2>✨ WORLD'S BEST BOYFRIEND ✨</h2>
            <p style="font-size: 18px;">Issued by: Diksha ❤️</p>
        </div>
    """, unsafe_allow_html=True)
...
# --- 5. Final Page (Hearts Falling) ---
    ...
    <div>
    <h3>My Jaan,</h3>
    <p style='font-size: 18px;'>In this new year, I promise to love you even more. You are my happiness and my whole world. 
    Thank you for being mine. Thank you so much for coming in my life ishuu....</p>
    <br>
    <b>I love u so much Ishri Pishri ❤️</b>
    </div>
    """, unsafe_allow_html=True)
